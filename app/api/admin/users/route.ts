...
export async function GET(_req: NextRequest) {
  ...
  if (!admin) return NextResponse.json({ data: null, error: { code: 'FORBIDDEN', message: '管理者のみアクセスできます' } }, { status: 403 })
...
export async function POST(req: NextRequest) {
  const admin = await requireAdmin()
  if (!admin) return NextResponse.json({ data: null, error: { code: 'FORBIDDEN', message: '管理者のみ作成できます' } }, { status: 403 })
  ...
  const { data } = await adminClient.from('users').select('*').eq('id', authData.user.id).single()
