...
function YouTubePageInner() {
  ...
  const filteredSchedules = (
    showUnreservedOnly
      ? schedules.filter((s) => s.video_length === 'long' && !s.post_reserved && s.status !== 'posted')
      : activeMonth
      ...
      {loadingAccounts ? (
        ...
        <>
          ...
          <div className="flex items-center gap-2">
            ...
            {/* 未予約長尺一覧ボタン */}
            <button
              onClick={() => setShowUnreservedOnly((v) => !v)}
              className={`inline-flex items-center gap-1.5 text-xs border rounded-lg px-2.5 py-1 transition-colors ${
                showUnreservedOnly
              ...
              <div className="divide-y divide-gray-100">
                {filteredSchedules.length === 0 ? (
                  <div className="px-4 py-10 text-center text-gray-400 italic text-sm">
                    {showUnreservedOnly ? '未予約の長尺動画はありません' : activeMonth ? `${formatMonthLabel(activeMonth)}のスケジュールがありません` : 'スケジュールがありません'}
                  </div>
                ) : (
